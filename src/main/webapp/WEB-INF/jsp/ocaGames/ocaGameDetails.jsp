...
<petclinic:layout pageName="ocaGames">
	...
	<c:forEach items="${ocaBoard.pieces}" var="piece">
		<c:choose>
			<c:when test='${ocaPiece.id == piece.id}'>
				<petclinic:currentOcaPiece size="80" piece="${piece}" />
			</c:when>
			<c:otherwise>
				<petclinic:ocaPiece size="80" piece="${piece}" />
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<div class="col-md-3">
		<c:choose>
			<c:when test="${player.id==ocaTurn.player.id}">
				...
				<a class="btn btn-primary" href="${fn:escapeXml(joinUrl)}">ROLL
					IT MOTHER FUCKER</a>
			</c:when>
			<c:otherwise>
				<p>NO ES TU TURNO</p>
